...
const URGENCY_CONFIG: Record<SnapshotTriageResult['urgency_tier'], { label: string; color: string; Icon: typeof CheckCircle }> = {
  rotina: { label: 'Tudo bem — cuidados de rotina', color: 'emerald', Icon: CheckCircle },
  acompanhar: { label: 'Acompanhar nos próximos dias', color: 'amber', Icon: Clock },
  agendar_vet: { label: 'Agendar veterinário', color: 'orange', Icon: AlertTriangle },
  vet_urgente: { label: 'Procurar veterinário urgente', color: 'red', Icon: AlertCircle },
}
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-surface-100 dark:border-surface-700 sticky top-0 bg-white/95 dark:bg-surface-800/95 backdrop-blur z-10">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-surface-900 dark:text-white">Triagem por foto — {petName}</h2>
          </div>
          ...
          {!result && (
            <>
              <p className="text-sm text-surface-600 dark:text-surface-300">
                Tire uma foto clara do {petName} — frontal, bem iluminada. A IA analisa olhos, boca, postura e dá uma triagem em segundos.
              </p>
              ...
              {!preview ? (
                <label className="block">
                  ...
                  <div className="border-2 border-dashed border-primary-200 dark:border-primary-700 hover:border-primary-400 dark:hover:border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-2xl p-8 text-center cursor-pointer transition">
                    ...
                    <p className="text-sm font-semibold text-surface-800 dark:text-white">Tirar foto ou escolher</p>
            ...
            <>
              {result.image_quality === 'ruim' ? (
                <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700/50 rounded-2xl p-4">
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-200 mb-1">Foto não está boa para análise</p>
                ...
                <>
                  <div className={cn(
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="font-bold text-surface-900 dark:text-white">{cfg.label}</p>
                    </div>
                  ...
                  {result.body_condition_score && (
                    <div className="bg-surface-50 dark:bg-surface-700/50 rounded-xl p-3">
                      <p className="text-xs uppercase tracking-wide text-surface-400 font-semibold mb-1">Body Condition Score (WSAVA)</p>
                  ...
                  {result.eyes.visible && result.eyes.concerns.length > 0 && (
                    <DetailBlock title="Olhos" severity={result.eyes.severity} items={result.eyes.concerns} />
                  )}
                  {result.dental.visible && (result.dental.tartar_level !== 'nenhum' || result.dental.concerns.length > 0) && (
                    <DetailBlock title="Dental" severity={result.dental.tartar_level} items={result.dental.concerns} />
                  )}
                  {result.skin_coat.concerns.length > 0 && (
                    <DetailBlock title="Pelagem e pele" severity={result.skin_coat.severity} items={result.skin_coat.concerns} />
                  )}

                  {result.recommendations.length > 0 && (
                    <div>
                      <p className="text-xs uppercase tracking-wide text-surface-400 font-semibold mb-2">Recomendações</p>
