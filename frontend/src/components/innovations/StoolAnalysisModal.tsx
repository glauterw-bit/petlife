...
const URGENCY_CONFIG = {
  rotina: { color: 'emerald', label: 'Normal', Icon: CheckCircle },
  acompanhar: { color: 'amber', label: 'Acompanhar', Icon: Clock },
  vet_agendar: { color: 'orange', label: 'Agendar vet', Icon: AlertTriangle },
  vet_urgente: { color: 'red', label: 'Vet urgente', Icon: AlertCircle },
} as const

const COLOR_LABEL: Record<string, string> = {
  marrom_claro: 'Marrom claro', marrom_escuro: 'Marrom escuro', amarelo: 'Amarelo',
  verde: 'Verde', preto_alcatrao: 'Preto (alcatrão)', avermelhado: 'Avermelhado',
  cinza: 'Cinza', outro: 'Outro',
}
      ...
      <div className="bg-white dark:bg-surface-800 rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg max-h-[92vh] overflow-y-auto animate-slide-up shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-surface-100 dark:border-surface-700 sticky top-0 bg-white/95 dark:bg-surface-800/95 backdrop-blur z-10">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-surface-900 dark:text-white">Análise de fezes — {petName}</h2>
          </div>
          ...
          {!result && (
            <>
              <p className="text-sm text-surface-600 dark:text-surface-300">
                Foto das fezes do {petName} em luz natural. A IA classifica na escala fecal veterinária (1-7) e identifica sinais de alerta.
              </p>
              {!preview ? (
                <label className="block">
                  ...
                  <div className="border-2 border-dashed border-amber-200 dark:border-amber-700 hover:border-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-2xl p-8 text-center cursor-pointer transition">
                    ...
                    <p className="text-sm font-semibold text-surface-800 dark:text-white">Tirar foto</p>
                  </div>
                </label>
              ) : (
            ...
            <>
              {result.image_quality === 'ruim' ? (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
                  <p className="font-semibold text-amber-800">Foto não está clara</p>
                ...
                <>
                  <div className={cn(
                    ...
                    <p className="text-xs uppercase tracking-wide text-surface-500 dark:text-surface-400 mb-1">Ideal: {result.ideal_range}</p>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-surface-50 dark:bg-surface-700/50 rounded-xl p-3">
                      <p className="text-xs uppercase tracking-wide text-surface-400 font-semibold">Cor</p>
                      <p className="text-sm font-semibold text-surface-900 dark:text-white mt-0.5">
                        {COLOR_LABEL[result.color] || result.color}
                      </p>
                      <p className="text-xs text-surface-500 dark:text-surface-400 mt-1">{result.color_notes}</p>
                    </div>
                    {result.fecal_score && (
                      <div className="bg-surface-50 dark:bg-surface-700/50 rounded-xl p-3">
                        <p className="text-xs uppercase tracking-wide text-surface-400 font-semibold">Score fecal</p>
                  ...
                  {result.alerts?.length > 0 && (
                    <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 rounded-xl p-3">
                      <p className="text-xs uppercase tracking-wide text-red-700 dark:text-red-300 font-bold mb-1.5">⚠️ Alertas</p>
