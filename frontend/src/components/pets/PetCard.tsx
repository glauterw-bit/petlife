...
export function PetCard({ pet, className = '' }: PetCardProps) {
  return (
    <Link
      href={`/pets/${pet.id}`}
      className={`block bg-white rounded-2xl border border-surface-100 hover:border-primary-300 hover:shadow-lg transition-all group overflow-hidden ${className}`}
    >
      {/* Photo */}
      <div className="relative h-44 bg-gradient-to-br from-primary-50 to-accent-50 flex items-center justify-center">
        ...
        <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-full px-2.5 py-0.5 text-xs font-medium text-surface-600 border border-surface-200">
          {getSpeciesLabel(pet.species)}
        </div>
        {pet.gender && (
          <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm rounded-full px-2.5 py-0.5 text-xs font-medium text-surface-600 border border-surface-200">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-2">
          <div>
            ...
            {pet.breed?.name && (
              <p className="text-sm text-surface-500">{pet.breed.name}</p>
            )}
          </div>
        ...
        {pet.neutered !== undefined && (
          <div className="mt-3">
            <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
              pet.neutered ? 'bg-green-50 text-green-700' : 'bg-surface-100 text-surface-500'
            }`}>
