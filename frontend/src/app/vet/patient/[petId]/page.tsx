      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700 p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="space-y-1 mt-1">
              <div className="text-sm text-surface-600 dark:text-surface-300"><span className="font-medium">Raça:</span> {pet.breed?.name ?? '—'}</div>
              <div className="text-sm text-surface-600 dark:text-surface-300"><span className="font-medium">Idade:</span> {formatAge(pet.birth_date)}</div>
              <div className="text-sm text-surface-600 dark:text-surface-300"><span className="font-medium">Peso:</span> {pet.weight ? `${pet.weight} kg` : '—'}</div>
              {pet.microchip && <div className="text-sm text-surface-600 dark:text-surface-300"><span className="font-medium">Microchip:</span> <span className="font-mono">{pet.microchip}</span></div>}
              {pet.neutered !== undefined && (
                <div className="text-sm text-surface-600 dark:text-surface-300"><span className="font-medium">Castrado:</span> {pet.neutered ? 'Sim' : 'Não'}</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700 p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-bold text-surface-900 dark:text-white">Dados do Tutor</h3>
          </div>
          <div className="space-y-2">
            <div className="text-sm text-surface-600 dark:text-surface-300"><span className="font-medium">Nome:</span> {owner.name}</div>
            <div className="text-sm text-surface-600 dark:text-surface-300"><span className="font-medium">E-mail:</span> {owner.email}</div>
            {owner.phone && (
      ...
      <div className="animate-fade-in">
        {tab === 'overview' && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={s.label} className={`rounded-2xl p-5 border border-surface-100 dark:border-surface-700 ${s.color.split(' ')[0]}`}>
            ...
            {pet.bio && (
              <div className="col-span-2 md:col-span-4 bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700 p-5">
                <h3 className="font-semibold text-surface-900 dark:text-white mb-2">Observações do Tutor</h3>
        ...
        {tab === 'vaccines' && (
          vaccines.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700">
              ...
              <p className="text-surface-500 dark:text-surface-400">Nenhuma vacina registrada.</p>
            </div>
          ) : (
        ...
        {tab === 'exams' && (
          exams.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700">
              <div className="text-5xl mb-3">🔬</div>
              <p className="text-surface-500 dark:text-surface-400">Nenhum exame registrado.</p>
            </div>
          ) : (
        ...
        {tab === 'anamnesis' && (
          anamnesis.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700">
              <div className="text-5xl mb-3">📋</div>
              <p className="text-surface-500 dark:text-surface-400">Nenhuma anamnese registrada.</p>
            </div>
          ) : (
              ...
              {anamnesis.map(a => {
                ...
                return (
                  <div key={a.id} className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700 p-5">
                    ...
                    <div className="text-sm font-medium text-surface-700 dark:text-surface-200 mb-1">Sintomas:</div>
        ...
        {tab === 'consultations' && (
          <div>
            ...
            {consultations.length === 0 ? (
              <div className="text-center py-12 bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700">
                <div className="text-5xl mb-3">🏥</div>
                <p className="text-surface-500 dark:text-surface-400">Nenhuma consulta registrada.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {consultations.map((c: Consultation) => (
                  <div key={c.id} className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700 p-5">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="font-semibold text-surface-900 dark:text-white">Consulta — {formatDate(c.date)}</span>
                      {c.follow_up_date && (
                    ...
                    {c.diagnosis && (
                      <div className="mb-2 text-sm">
                        <span className="font-medium text-surface-700 dark:text-surface-200">Diagnóstico: </span>
                        <span className="text-surface-600 dark:text-surface-300">{c.diagnosis}</span>
                      </div>
                    )}
                    {c.treatment && (
                      <div className="mb-2 text-sm">
                        <span className="font-medium text-surface-700 dark:text-surface-200">Tratamento: </span>
      ...
      <Modal open={showConsultModal} onClose={() => setShowConsultModal(false)} title="Registrar Consulta" size="lg">
        <form onSubmit={handleAddConsultation} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-surface-700 dark:text-surface-200 mb-1.5">Data da consulta *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-surface-700 dark:text-surface-200 mb-1.5">Data de retorno</label>
          ...
          <div>
            <label className="block text-sm font-medium text-surface-700 dark:text-surface-200 mb-1.5">Diagnóstico</label>
            <textarea value={consultForm.diagnosis} onChange={set('diagnosis')} rows={3} placeholder="Diagnóstico do veterinário..." className="w-full px-4 py-3 border border-surface-200 dark:border-surface-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-surface-700 dark:text-surface-200 mb-1.5">Tratamento prescrito</label>
            <textarea value={consultForm.treatment} onChange={set('treatment')} rows={3} placeholder="Medicamentos, procedimentos, recomendações..." className="w-full px-4 py-3 border border-surface-200 dark:border-surface-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none" />
          </div>
          <div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={() => setShowConsultModal(false)} className="flex-1 py-3 border border-surface-200 dark:border-surface-700 rounded-xl text-sm font-medium text-surface-700 dark:text-surface-200 hover:bg-surface-50 dark:hover:bg-surface-700/40 transition">Cancelar</button>
