  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total', value: stats.total, color: 'bg-surface-100 text-surface-700' },
          ...
          <div key={s.label} className={`rounded-2xl p-4 border border-surface-100 ${s.color.split(' ')[0]}`}>
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-sm font-medium transition ${filterPet === '' ? 'bg-primary-500 text-white' : 'bg-white border border-surface-200 text-surface-700 hover:border-primary-300'}`}
            >
              Todos os pets
            </button>
            {petList.map(p => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-sm font-medium transition ${filterPet === p.id ? 'bg-primary-500 text-white' : 'bg-white border border-surface-200 text-surface-700 hover:border-primary-300'}`}
              >
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Nova Vacina" size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-surface-700 mb-1.5">Nome da vacina *</label>
            <input required type="text" value={form.name} onChange={set('name')} placeholder="Ex: Antirrábica, V10, Giárdia..." className="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-surface-700 mb-1.5">Data de aplicação *</label>
              <input required type="date" value={form.date_applied} onChange={set('date_applied')} className="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white" />
            </div>
            <div>
              <label className="block text-sm font-medium text-surface-700 mb-1.5">Próxima dose</label>
              <input type="date" value={form.next_due_date} onChange={set('next_due_date')} className="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-surface-700 mb-1.5">Número do lote</label>
              <input type="text" value={form.lot_number} onChange={set('lot_number')} placeholder="Lote da vacina" className="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
            </div>
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-surface-700 mb-1.5">Documento / Comprovante</label>
            <input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={e => setDocFile(e.target.files?.[0] ?? null)} className="w-full text-sm text-surface-600 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-primary-50 file:text-primary-700 file:font-medium hover:file:bg-primary-100" />
          </div>
