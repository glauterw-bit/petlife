...
const statusConfig = {
  up_to_date: { label: 'Em dia', color: 'bg-emerald-100 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500', Icon: CheckCircle, iconColor: 'text-emerald-500' },
  upcoming: { label: 'Próxima', color: 'bg-amber-100 text-amber-700 border-amber-200', dot: 'bg-amber-500', Icon: Clock, iconColor: 'text-amber-500' },
  overdue: { label: 'Atrasada', color: 'bg-red-100 text-red-700 border-red-200', dot: 'bg-red-500', Icon: AlertCircle, iconColor: 'text-red-500' },
}
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!res.ok) throw new Error('Erro ao carregar carteirinha')
          ...
          <div className="px-8 py-6 border-t border-surface-100 dark:border-surface-700 bg-surface-50/50 flex items-end justify-between gap-6 flex-wrap">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-surface-400 font-medium uppercase tracking-wide">Código de verificação</span>
              </div>
              <img
                src={qrUrl}
