...
const DAYS_OF_WEEK = ['g.rt.dow0', 'g.rt.dow1', 'g.rt.dow2', 'g.rt.dow3', 'g.rt.dow4', 'g.rt.dow5', 'g.rt.dow6']
...
export default function RoutinesPage() {
  ...
  async function handleGenerate(petId: number) {
    ...
    try {
      ...
      success(t('g.rt.generated'))
    } catch { error(t('g.rt.errGenerate')) }
  ...
  return (
    <DashboardLayout>
      <div className="mb-5 md:mb-6 ">
        <h1 className="text-2xl md:text-3xl font-bold text-surface-900 dark:text-white leading-tight">{t('g.rt.title')}</h1>
        <p className="text-sm md:text-base text-surface-500 dark:text-surface-400 mt-1">{t('g.rt.subtitle')}</p>
      </div>

      {loading ? <PageLoader /> : petList.length === 0 ? (
        <div className="text-center py-20">
          <div className="text-6xl mb-3">🦮</div>
          <p className="text-surface-700 dark:text-surface-200 font-medium mb-2">{t('g.rt.noPets')}</p>
          <a href="/pets/new" className="text-primary-600 hover:underline text-sm">{t('g.rt.registerPet')}</a>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          {/* Pet list */}
          <div className="space-y-2">
            <p className="text-sm font-medium text-surface-500 dark:text-surface-400 mb-3">{t('g.rt.yourPets')}</p>
            {petList.map(p => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-surface-500 dark:text-surface-400">{hasRoutine ? t('g.rt.hasRoutine') : t('g.rt.noRoutine')}</div>
                  </div>
                </button>
              )
            })}
              ...
              <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-100 dark:border-surface-700 p-6">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-surface-500 dark:text-surface-400">{activePet.breed?.name ?? t('g.rt.noBreed')}</p>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    ...
                    {activeRoutine ? t('g.rt.regenerate') : t('g.rt.generateAi')}
                  </button>
                </div>
                ...
                {!activeRoutine ? (
                  <div className="text-center py-12 bg-surface-50 dark:bg-surface-900/60 rounded-2xl">
                    ...
                    <p className="text-surface-600 dark:text-surface-300 mb-4">{t('g.rt.emptyTitle', { name: activePet.name })}</p>
                    <p className="text-sm text-surface-500 dark:text-surface-400">{t('g.rt.emptyText')}</p>
                  </div>
                ) : (
                  <>
                    {/* Stats */}
                    ...
                    {/* Weekly grid */}
                    <div className="mb-6">
                      <h3 className="text-sm font-semibold text-surface-700 dark:text-surface-200 mb-3">{t('g.rt.weeklyGrid')}</h3>
                    ...
                    {activeRoutine.exercise_type && (
                      <div className="mb-6 p-3 bg-surface-50 dark:bg-surface-900/60 rounded-xl">
                        <span className="text-sm font-medium text-surface-700 dark:text-surface-200">{t('g.rt.exerciseType')} </span>
